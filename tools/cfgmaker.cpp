...
int main(int argc, char *argv[])
{
    ...
    outs.precision(4);
    // title
    outs << sysname << " " << nspecie << endl;
    // atoms
    outs <<  natom << "  atoms ";
    for (ii=0;ii<nspecie;ii++)
	outs << nmole_per_specie[ii] << " " << natom_per_mole[ii] << " ";
    outs << endl;
    iatom_counter = 0;
    imole_counter = 0;
    for (ii=0;ii<nspecie;ii++)
    ...
    outs << nbond << "  bonds ";
    for (ii=0;ii<nspecie;ii++)
	outs << ((SampleSpecie+ii)->sample_mole).nbond << " ";
    outs << endl;
    for (ii=0;ii<nspecie;ii++)
    {
	pMole = &((SampleSpecie+ii)->sample_mole);
	for (jj=0;jj<nmole_per_specie[ii];jj++)
	{
		...
		outs << pBond->comp_id[0]+jj*pMole->natom << ' '
		    << pBond->comp_id[1]+jj*pMole->natom << ' '
		    << bond_type[ii] << ' '
		    << pBond->kr << ' '
		    << pBond->req << ' '
		    << pBond->alpha << endl;
    ...
    outs << nangle << "  angles ";
    for (ii=0;ii<nspecie;ii++)
	outs << ((SampleSpecie+ii)->sample_mole).nangle << " ";
    outs << endl;
    for (ii=0;ii<nspecie;ii++)
    {
	pMole = &((SampleSpecie+ii)->sample_mole);
       	for (jj=0;jj<nmole_per_specie[ii];jj++)
	{
		...
		outs << pAngle->comp_id[0]+jj*pMole->natom << ' '
		    << pAngle->comp_id[1]+jj*pMole->natom << ' '
		    << pAngle->comp_id[2]+jj*pMole->natom << ' '
		    << angle_type[ii] << ' '
		    << pAngle->k_theta << ' '
		    << pAngle->theta_eq << ' '
		    << pAngle->agl_para_3 << ' '
    ...
    outs << ndih << "  dihedrals ";
    for (ii=0;ii<nspecie;ii++)
	outs << ((SampleSpecie+ii)->sample_mole).ndih << " ";
    outs << endl;
    for (ii=0;ii<nspecie;ii++)
    {
	pMole = &((SampleSpecie+ii)->sample_mole);
       	for (jj=0;jj<nmole_per_specie[ii];jj++)
	{
		...
		outs << pDih->comp_id[0]+jj*pMole->natom << ' '
		    << pDih->comp_id[1]+jj*pMole->natom << ' '
		    << pDih->comp_id[2]+jj*pMole->natom << ' '
		    << pDih->comp_id[3]+jj*pMole->natom << ' '
		    << dih_type[ii] << ' '
		    << pDih->v1 << ' '
		    << pDih->v2 << ' '
		    << pDih->v3 << ' '
    ...
    outs << nimp << "  impropers ";
    for (ii=0;ii<nspecie;ii++)
	outs << ((SampleSpecie+ii)->sample_mole).nimp << " ";
    outs << endl;

    // nonbonded
    outs << nnbp << "   nonbonded ";
    for (ii=0;ii<nspecie;ii++)
	outs << ((SampleSpecie+ii)->sample_mole).nnonpair << " ";
    outs << endl;
    for (ii=0;ii<nspecie;ii++)
    {
	pMole = &((SampleSpecie+ii)->sample_mole);
	for (jj=0;jj<nmole_per_specie[ii];jj++)
	{
		...
		outs << pNonpair->comp_id[0]+jj*pMole->natom << ' '
		    << pNonpair->comp_id[1]+jj*pMole->natom << endl;
	    }
	}
    }
